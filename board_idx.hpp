...
//ハッシュ関数
   struct hash {
    ...
    //合法判定を行う関数
    inline bool legal_place(int g_place)const
    {
        int line,p;
        for(int i=0;i<4;i++){
...
inline int count_vacant(){
    int res=0;
    int b=0,w=0;
    for(int line = 0;line < hw ;line++ ){
    b = create_one_color(board_idx[line],0);
    w = create_one_color(board_idx[line],1);
    for(int i = 0; i < hw;i++){
        res += (( 1&(b>>(8-1-i)) )+( 1&(w>>(8-1-i)) ) );
    }
    }
    return 64-res;
};
